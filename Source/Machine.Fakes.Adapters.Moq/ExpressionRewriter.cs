...
namespace Machine.Fakes.Adapters.Moq
{
    public class ExpressionRewriter : AbstractExpressionRewriter
    {
        ...
        static Expression RewriteMatchesMethod(MethodCallExpression expression)
        {
            ...
            {
                methodTypeParameter = method.GetFirstTypeArgument();
            }

            if (matchExpression is UnaryExpression)
            {
                matchExpression = ((UnaryExpression) matchExpression).Operand;
                methodTypeParameter = method.DeclaringType.GetFirstTypeArgument();
            }
